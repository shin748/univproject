...
class excel:
    ...
    def set_all_plot(self, save=False, bas=True, preprocess=False, interpolate=False, cycle=1, opt=False):
        ...
        for i in range(plen):
            ...
            rtfc_s, rtfc_e = self.df['TFC_start_'+series][k].astype(int), self.df['TFC_end_'+series][k].astype(int)
            rccfc_s, rccfc_e = self.df['CCFC_start_'+series][k].astype(int), self.df['CCFC_end_'+series][k].astype(int)
            ...
            ##엑셀속 frame count 표시
            plt.axvspan(self.drop_df['TFC_start_'+series][k].astype(int), self.drop_df['TFC_end_'+series][k].astype(int), facecolor='red')
            plt.axvspan(self.drop_df['CCFC_start_'+series][k].astype(int), self.drop_df['CCFC_end_'+series][k].astype(int), facecolor='blue')
    ...
    def show_plot(self, pat_num, bas=True, preprocess=False, interpolate=False, cycle=1, opt=False):  
        pat_num-=1
        ...
        if interpolate == False: plt.plot(mat_i, mat_x, color='black')
            ...
            for i in range(cycle):
                interp_i, mat_x1 = interp().peak_interpolation(mat_i, mat_x) #1차 보간
                TFCS, TFCE, CCFCS, CCFCE = analysis().get_TFC_CCFC(np.array(interp_i), np.array(mat_x1), isgmm=True, draw=True) #클러스터링(n=2)기반 극대극소를 발견하는 TFC
                #TFCS, TFCE, CCFCS, CCFCE = analysis().get_TFC_CCFC(np.array(mat_i), np.array(mat_x), isgmm=True, draw=True) #클러스터링(n=2)기반 극대극소를 발견하는 TFC
        ...
        ##엑셀속 frame count 표시
        plt.axvspan(self.df['TFC_start_'+series][k].astype(int), self.df['TFC_end_'+series][k].astype(int), facecolor='red')
        plt.axvspan(self.df['CCFC_start_'+series][k].astype(int), self.df['CCFC_end_'+series][k].astype(int), facecolor='blue')
        plt.show()
    #######################################################
